@extends('layouts.main', ['title' => 'Distress - INSAF'])

@section('content')
<div class="flex mb-6">
    <h1 class="text-2xl font-bold">Distress</h1>
</div>
<div class="p-6 rounded-lg bg-white w-full h-auto" x-data="{ open: false }">
    <div class="block space-y-5 lg:space-y-0 lg:flex items-center justify-between mb-5 lg:w-full">
        ...
        <div class="w-full lg:w-1/2 flex items-center justify-end">
            <a href="{{route('distress_create.insaf')}}" class="btn-table-add">
    ...
    <div class="overflow-x-auto overflow-y-auto">
        <div class="my-6">
            <table id="table-3" class="table table-auto my-3" data-ordering="true" data-paging="true"  data-searching="true">
                <thead class="">
                    <tr>
                        <th class="text-left">No Distress</th>
                        <th class="text-left">Waktu Kejadian</th>
                        <th class="text-left">Judul Distress</th>
                ...
                <tbody class="">
                    @if ($data_distress)
                        @foreach($data_distress as $loopdata) 
                            <tr>
                                <td class="">
                                    <a href="{{route('distress_show.insaf', $loopdata['id'])}}">
										{{ $loopdata["no_jurnal"] }}
                                    </a>
                                </td>
                                <td>
									<a href="{{route('distress_show.insaf', $loopdata['id'])}}">
										{{ \Carbon\carbon::parse(strtotime($loopdata["tanggal"]))->setTimezone('Asia/Jakarta')->translatedFormat('d F Y') }}
									</a>
                                </td>
                                <td>
									<a href="{{route('distress_show.insaf', $loopdata['id'])}}">
										{{ $loopdata["judul_distress"] }}
									</a>
                                </td>
                                <td>
                                    <a href="{{route('distress_show.insaf', $loopdata['id'])}}">
										{{ Str::limit($loopdata["deskripsi_assesment"], 30) }}
									</a>
                                </td>
                                <td class="w-40 ">
                                    <div class="flex items-center justify-center space-x-2 text-center">
                                        <a href="{{route('distress_edit.insaf', $loopdata['id'])}}" class="cta-edit-table">
                                            ...
                                            <div x-show="open" @click.away="open = false"  x-init="setTimeout(() => show = false, 8000)" id="{{$loopdata['id']}}"
                                                ...
                                                <div class="modal relative bg-white overflow-hidden rounded-xl w-96 h-96 z-50">
                                                    ...
                                                    <div class="p-5 flex flex-col items-center justify-center modal-body">
                                                        ...
                                                        <h3 class="text-lg font-bold">Hapus Data Distress</h3>
                                                    ...
                                                    <div class="modal-footer absolute inset-x-0 bottom-0 bg-gray-100 flex items-center justify-center py-3 space-x-2">
														...
														<a href="{{ route('distress_delete.insaf', [$loopdata['id']]) }}" class="transition duration-100 focus:outline-none focus:ring-4 focus:ring-red-400 focus:bg-red-500 bg-red-600 hover:bg-red-500 text-white font-medium hover:text-white  flex items-center justify-center py-2 px-5 rounded-lg">
                                                            Hapus
                                                        </a>
                                                    </div>
                                                </div>
